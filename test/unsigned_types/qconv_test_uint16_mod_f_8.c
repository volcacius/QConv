...
enum qconv_status qconv_test_NTT_1D_random_identity_mod_f_8(size_t size, size_t bit_size) {
    ...
    for (int i = 0; i < TEST_ITERATIONS; i++) {
        ...
        status = qconv_NTT_1D_uint16_mod_f_8(size, a, optimize_null);
        CHECK_TEST_STATUS(status);
        status = qconv_INTT_1D_uint16_mod_f_8(size, a, optimize_null);
        CHECK_TEST_STATUS(status);
...
enum qconv_status qconv_test_NTT_2D_random_identity_mod_f_8(size_t size_width,
    ...
    for (int i = 0; i < TEST_ITERATIONS; i++) {
        ...
        status = qconv_NTT_2D_uint16_mod_f_8(size_width, size_height, random, optimize_null);
        CHECK_TEST_STATUS(status);
        status = qconv_INTT_2D_uint16_mod_f_8(size_width, size_height, random, optimize_null);
        CHECK_TEST_STATUS(status);
...
enum qconv_status qconv_test_NTT_1D_max_circular_convolution_mod_f_8(size_t size, size_t input_bit_size, size_t kernel_bit_size) {
    ...
    status = qconv_NTT_1D_circular_convolution_uint16_mod_f_8(size, input, kernel, ntt, optimize_null);
    CHECK_TEST_STATUS(status);
