...
enum qconv_status qconv_test_NTT_1D_max_identity_mod_f_3(size_t size, size_t bit_size) {
    ...
    status = qconv_NTT_1D_uint16_mod_f_3(size, max, optimize_null);
    CHECK_TEST_STATUS(status);
    status = qconv_INTT_1D_uint16_mod_f_3(size, max, optimize_null);
    CHECK_TEST_STATUS(status);
    status = qconv_NTT_1D_uint16_mod_f_3(size, max_precomp, optimize_precomp);
    CHECK_TEST_STATUS(status);
    status = qconv_INTT_1D_uint16_mod_f_3(size, max_precomp, optimize_precomp);
    CHECK_TEST_STATUS(status);
...
enum qconv_status qconv_test_NTT_2D_max_identity_mod_f_3(size_t size_width,
    ...
    status = qconv_NTT_2D_uint16_mod_f_3(size_width, size_height, max, optimize_null);
    CHECK_TEST_STATUS(status);
    status = qconv_INTT_2D_uint16_mod_f_3(size_width, size_height, max, optimize_null);
    CHECK_TEST_STATUS(status);
    status = qconv_NTT_2D_uint16_mod_f_3(size_width, size_height, max_precomp, optimize_precomp);
    CHECK_TEST_STATUS(status);
    status = qconv_INTT_2D_uint16_mod_f_3(size_width, size_height, max_precomp, optimize_precomp);
    CHECK_TEST_STATUS(status);
...
enum qconv_status qconv_test_NTT_1D_random_identity_mod_f_3(size_t size, size_t bit_size) {
    enum qconv_status status;
    ...
    for (int i = 0; i < TEST_ITERATIONS; i++) {
        ...
        status = qconv_NTT_1D_uint16_mod_f_3(size, a, optimize_null);
        CHECK_TEST_STATUS(status);
        status = qconv_INTT_1D_uint16_mod_f_3(size, a, optimize_null);
        CHECK_TEST_STATUS(status);
...
enum qconv_status qconv_test_NTT_1D_max_circular_convolution_mod_f_3(size_t size, size_t input_bit_size, size_t kernel_bit_size) {
    enum qconv_status status;
    ...
    status = qconv_NTT_1D_circular_convolution_uint16_mod_f_3(size, input, kernel, ntt, optimize_null);
    CHECK_TEST_STATUS(status);
...
enum qconv_status qconv_test_NTT_2D_max_circular_convolution_mod_f_3(size_t size_width,
    ...
    CHECK_TEST_STATUS(status);
    //NTT linear convolution
    status = qconv_NTT_2D_circular_convolution_uint16_mod_f_3(size_width, size_height, input, kernel, ntt, optimize_precomp);
    CHECK_TEST_STATUS(status);
    assert(qconv_test_util_compare_uint16_2D_array(size_width, size_height, ntt, conv));
    return status_success;
}

enum qconv_status qconv_test_NTT_1D_random_circular_convolution_mod_f_3(size_t size, size_t input_bit_size, size_t kernel_bit_size) {
    enum qconv_status status;
    ...
    for (int i = 0; i < TEST_ITERATIONS; i++) {
        ...
        status = qconv_NTT_1D_circular_convolution_uint16_mod_f_3(size, a, b, ntt, optimize_precomp);
...
enum qconv_status qconv_test_NTT_1D_max_linear_convolution_mod_f_3(size_t input_size,
    ...
    CHECK_TEST_STATUS(status);
    //NTT linear convolution
    status = qconv_NTT_1D_linear_convolution_uint16_mod_f_3(input_size, kernel_size, input, kernel, ntt, optimize_precomp);
    CHECK_TEST_STATUS(status);
...
enum qconv_status qconv_test_NTT_2D_max_linear_convolution_mod_f_3(size_t input_size_width,
    ...
    CHECK_TEST_STATUS(status);
    //NTT linear convolution
    status = qconv_NTT_2D_linear_convolution_uint16_mod_f_3(input_size_width, input_size_height, kernel_size_width, kernel_size_height, input, kernel, ntt, optimize_precomp);
    CHECK_TEST_STATUS(status);
...
enum qconv_status qconv_test_NTT_1D_identity_mod_f_3(size_t size, size_t bit_size) {
    enum qconv_status status;
    status = qconv_test_NTT_1D_max_identity_mod_f_3(size, bit_size);
    CHECK_TEST_STATUS(status);
    status = qconv_test_NTT_1D_random_identity_mod_f_3(size, bit_size);
    return status;
}

enum qconv_status qconv_test_NTT_2D_identity_mod_f_3(size_t size_width, size_t size_height, size_t bit_size) {
    enum qconv_status status;
    status = qconv_test_NTT_2D_max_identity_mod_f_3(size_width, size_height, bit_size);
    CHECK_TEST_STATUS(status);
    status = qconv_test_NTT_2D_random_identity_mod_f_3(size_width, size_height, bit_size);
    return status;
}
...
enum qconv_status qconv_test_NTT_1D_circular_convolution_mod_f_3(size_t size,
                                                    ...
                                                    size_t kernel_bit_size) {
    enum qconv_status status;
    status = qconv_test_NTT_1D_max_circular_convolution_mod_f_3(size, input_bit_size, kernel_bit_size);
    CHECK_TEST_STATUS(status);
    status = qconv_test_NTT_1D_random_circular_convolution_mod_f_3(size, input_bit_size, kernel_bit_size);
    return status;
}
...
enum qconv_status qconv_test_NTT_2D_circular_convolution_mod_f_3(size_t size_width,
                                                    ...
                                                    size_t kernel_bit_size) {
    enum qconv_status status;
    status = qconv_test_NTT_2D_max_circular_convolution_mod_f_3(size_width, size_height, input_bit_size, kernel_bit_size);
    CHECK_TEST_STATUS(status);
    status = qconv_test_NTT_2D_random_circular_convolution_mod_f_3(size_width, size_height, input_bit_size, kernel_bit_size);
    return status;
}
...
enum qconv_status qconv_test_NTT_1D_linear_convolution_mod_f_3(size_t input_size,
                                                  ...
                                                  size_t kernel_bit_size) {
    enum qconv_status status;
    status = qconv_test_NTT_1D_max_linear_convolution_mod_f_3(input_size, kernel_size, input_bit_size, kernel_bit_size);
    CHECK_TEST_STATUS(status);
    status = qconv_test_NTT_1D_random_linear_convolution_mod_f_3(input_size, kernel_size, input_bit_size, kernel_bit_size);
    return status;
}
...
enum qconv_status qconv_test_NTT_1D_identity_mod_f_3_runall() {
    ...
    for (size_t bit_size = 1; bit_size <= QCONV_MAX_F_8_NTT_IDENTITY_BITSIZE; bit_size++) {
        for (size_t i = 0; i < QCONV_F_8_1D_CONV_SIZES; i++) {
            status = qconv_test_NTT_1D_identity_mod_f_3(qconv_test_NTT_circular_convolution_uint16_mod_f_3_sizes[i], bit_size);
            CHECK_TEST_STATUS(status);
        }
    }
    return status_success;
    ...
    for (size_t bit_size = 1; bit_size <= QCONV_MAX_F_8_NTT_IDENTITY_BITSIZE; bit_size++) {
        for (size_t i = 0; i < QCONV_F_8_2D_CONV_SIZES; i++) {
            for (size_t j = 0; j < QCONV_F_8_2D_CONV_SIZES; j++) {
                status = qconv_test_NTT_2D_identity_mod_f_3(qconv_test_NTT_circular_convolution_uint16_mod_f_3_sizes[i],
                                                   qconv_test_NTT_circular_convolution_uint16_mod_f_3_sizes[j],
                                                   bit_size);
                CHECK_TEST_STATUS(status);
            }
        }
...
enum qconv_status qconv_test_NTT_1D_linear_convolution_mod_f_3_runall() {
    enum qconv_status status;
    status = qconv_test_NTT_1D_linear_convolution_mod_f_3(QCONV_LEN_16, QCONV_LEN_16, 2, 2);
    CHECK_TEST_STATUS(status);
    status = qconv_test_NTT_1D_linear_convolution_mod_f_3(QCONV_LEN_32, QCONV_LEN_32, 2, 1);
    CHECK_TEST_STATUS(status);
    status = qconv_test_NTT_1D_linear_convolution_mod_f_3(QCONV_LEN_64, QCONV_LEN_64, 2, 1);
    CHECK_TEST_STATUS(status);
    status = qconv_test_NTT_1D_linear_convolution_mod_f_3(QCONV_LEN_128, QCONV_LEN_32, 2, 1);
    return status;
}

enum qconv_status qconv_test_NTT_1D_circular_convolution_mod_f_3_runall() {
    enum qconv_status status;
    status = qconv_test_NTT_1D_circular_convolution_mod_f_3(QCONV_LEN_16, 2, 2);
    CHECK_TEST_STATUS(status);
    status = qconv_test_NTT_1D_circular_convolution_mod_f_3(QCONV_LEN_32, 2, 1);
    CHECK_TEST_STATUS(status);
    status = qconv_test_NTT_1D_circular_convolution_mod_f_3(QCONV_LEN_64, 2, 1);
    return status;
}

enum qconv_status qconv_test_NTT_2D_circular_convolution_mod_f_3_runall() {
    enum qconv_status status;
    status = qconv_test_NTT_2D_circular_convolution_mod_f_3(QCONV_LEN_8, QCONV_LEN_8, 2, 1);
    return status;
}

enum qconv_status qconv_test_NTT_2D_linear_convolution_mod_f_3_runall() {
    enum qconv_status status;
    status = qconv_test_NTT_2D_linear_convolution_mod_f_3(QCONV_LEN_8, QCONV_LEN_8, QCONV_LEN_8, QCONV_LEN_8, 2, 1);
    return status;
}
