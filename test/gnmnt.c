...
unsigned long long int mods(long long a, unsigned int p) {
    long long reduced = a % p;
    while (reduced < 0) {
        reduced += p;
    }
    return (unsigned long long int) reduced;
}

// Generate beta1 and beta2 for transforms
void betaGen(unsigned int N, unsigned int fN, unsigned int fNmm,
             unsigned long long *B1, unsigned long long *B2, unsigned long long *C1, unsigned long long *C2,
    ...
    for (size_t loop = 0; loop < (p - 2); loop++) {
        a1 = mods(a1 * a1, p);
        a2 = mods(a2 * a2, p);
    }
    for (size_t loop = 0; loop < d; loop++) {
    ...
    A1[0] = 1;
    A2[0] = 0;
    for (size_t loop = 0; loop < fNmm; loop++) {
...
void bitrevorder(unsigned long long *vec, unsigned int log2N) {
...
void nmnt(unsigned int N, unsigned int log2N, unsigned int p, unsigned long long *oldvec, unsigned long long *vec,
          unsigned long long *b1, unsigned long long *b2, unsigned long long inverse) {
    ...
    for (j = 1; j <= log2N; j++) {
        for (k = 0; k < (ts & tmask); k++) {
            ...
            for (n = 0; n < ((1 << (j - 2))); n++) {
                ...
                vec[line1] = (unsigned long long int) t1;
                vec[line2] = (unsigned long long int) t2;
                line1++;
                line2 = offset - n;
            }
        }
...
void onmnt(unsigned int N, unsigned int log2N, unsigned int p,unsigned long long *oldvec,
           unsigned long long *vec, unsigned long long *b1, unsigned long long *b2) {
...
void ionmnt(unsigned int N, unsigned int log2N, unsigned int p, unsigned int sn, unsigned long long *oldvec,
            unsigned long long *vec, unsigned long long *b1, unsigned long long *b2) {
    unsigned int i, j, k, n, io2, ts, tmask, line1, line2, betaz, step;
    long long t1, t2;
    i = N;
...
void o2nmnt(unsigned int N, unsigned int log2N, unsigned int p, unsigned long long *oldvec, unsigned long long *vec,
            unsigned long long *b1, unsigned long long *b2, unsigned long long *e1, unsigned long long *e2, unsigned long long inverse) {
    unsigned int i, j, k, n, io2, ts, tmask, line1, line2, betaz, step;
    long long t1, t2;
    i = 2;
    io2 = 1;
