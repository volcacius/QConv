...
enum qconv_status qconv_uint32_direct_2D_circular_convolution (size_t size_x,
                                                              size_t size_y,
            ...
            for (int i_x = 0; i_x < size_x; i_x++) {
                for (size_t i_y = 0; i_y < size_y; i_y++) {
                    ...
                    if (k_x >= 0 && k_y >= 0 && k_x < size_x && k_y < size_y) {
                        output[o_x][o_y].uint32.value += input[i_x][i_y].uint32.value * kernel[i_x][i_y].uint32.value;
                    }
                }
            }
        }
